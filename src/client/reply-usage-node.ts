/**
 * Per-turn provider-reported token usage for the reply-usage badge:
 ...
 *
 * Client-side (browser) folding: the conversation engine replays the full
 * session log when the session opens, so the fold is complete and stable
 * across host restarts / plugin reloads — unlike the previous host-side
 * in-memory fold, which only witnessed live `session/event` publications,
 * lost every step that predated the plugin (re)load, and collapsed the badge
 * to the last request's volume.
 */
...
interface ReplyUsageState {
  ...
  readonly steps: ReadonlyMap<number, UsageRecord>
  /** The turn's final usage sample — the last request's reported volume. */
  readonly last: UsageRecord | undefined
}
...
function usageOf(event: DefinitionEvent): UsageRecord | undefined {
...
function foldSteps(steps: ReadonlyMap<number, UsageRecord>): UsageRecord | undefined {
 ...
 * turn has any. Turn-scoped only; the badge reads it off the closing
 * message's turn via the conversation snapshot.
 */
